...
CausalE=function(otu.com,alpha.estimation,beta.estimation,covariate.fix)
{
  otu.num=ncol(otu.com)

  if(is.null(covariate.fix)) q=0 else q=length(covariate.fix)

   cov.1=c(1,covariate.fix)
   cov.2=c(1,covariate.fix,1)
